...
> ⚠️ **업무 상황은 가상입니다.** 작성자는 현재 실무가 없는 수강생이고, 캠프 운영 업무를
> 가상으로 설정했습니다([PROBLEM.md 0절](PROBLEM.md#0-도메인을-이렇게-고른-맥락)).
> **아래 3·5절의 측정값은 실제로 잰 숫자이고, 4절의 "기존 방식"은 가정값입니다.**
...
**이 표의 왼쪽 열은 전부 가정값입니다.** 실제 담당자를 관찰한 것이 아니라 일반적인 수작업 상황에서 잡은 값입니다. 그러므로 이 표는 **"개선되었다"의 증거가 아니라 "이 정도 차이면 확인해 볼 값이 있다"의 근거**로만 읽어야 합니다.

**시간 비교는 공정하지 않다는 점도 밝힙니다.** PoC의 0.044초는 **탐지만** 걸린 시간이고, 사람이 확인·가감하는 시간은 빠져 있습니다. 이 도구는 최종 판정을 대신하지 않으므로 실제 절감분은 "찾는 시간"에 한정됩니다. 정확한 비교는 담당자에게 후보가 표시된 상태로 200장을 처리하게 해서 재야 합니다. **아직 재지 않았습니다.**
...
**4. 성공 기준의 근거가 비어 있습니다. 이게 가장 큰 한계입니다.** 재현율 95%의 근거인 "현행 누락률 5%"는 **가정값입니다.** 업무 상황 자체가 가상이라 현행 수준을 잴 대상이 없었습니다([PROBLEM.md 0-3절](PROBLEM.md#0-3-무엇이-가정이고-무엇이-실제인가)).

그래서 **"성공 기준 통과"를 "도입할 값이 있다"로 읽으면 안 됩니다.** 사람이 실제로 2%만 놓친다면 기준은 98%여야 하고, 그러면 96.7%는 미달입니다. 기준선을 어디에 긋느냐에 따라 같은 숫자가 통과도 되고 탈락도 됩니다. **이 PoC가 확인한 것은 "얼굴 탐지로 후보를 찾는 것이 기술적으로 된다"까지입니다.**
